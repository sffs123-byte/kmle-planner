...
def load_source() -> tuple[list[dict], list[str], dict, dict, str, str]:
    ...
    for disease in diseases:
        key = disease.get("audioKey") or disease["id"]
        filename = "normal.mp3" if key == NORMAL_AUDIO_KEY else f"{disease['id']}.mp3"
        audio[key] = data_uri(ASSET_DIR / filename, "audio/mp3")
...
function renderCircles(){{
  ...
  el.innerHTML = results.map((r, i) => `<div class="circle ${{r ? 'correct' : 'wrong'}}">${{i+1}}</div>`).join('') +
...
function updatePosUI(){{
  POSITIONS.forEach((p, i) => {{
    ...
    el.className = i === curPos ? 'pos-btn active' : i < curPos ? 'pos-btn played' : 'pos-btn';
  }});
  document.getElementById('q-position-label').textContent = `${{POSITIONS[curPos]}} 위치 청진 중`;
  document.getElementById('btn-next-pos').disabled = curPos >= POSITIONS.length - 1 || answered;
  document.getElementById('btn-kw').disabled = answered;
}}

function playPos(){{
  playAudioSegmentByKey(currentAudioKey(current), POSITIONS[curPos]);
}}
